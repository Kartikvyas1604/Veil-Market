    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pt-10 pb-16 md:px-6 md:pt-14 lg:px-8">
        ...
        <div className="mb-8 stagger-enter" style={{ animationDelay: "0ms" }}>
          <h1 className="font-serif text-3xl text-veil-text-bright md:text-4xl">
            Markets
          </h1>
          <p className="mt-2 font-mono text-xs text-veil-text-muted">
        ...
        {/* Filter bar */}
        <div
          className="mb-8 flex gap-1 stagger-enter"
        ...
        >
          {filters.map((f) => (
            <button
              ...
              className={cn(
                "relative rounded-md px-4 py-2 font-mono text-xs tracking-wide transition-colors duration-150",
                ...
                filter === f.key
                  ? "text-veil-accent bg-veil-surface"
                  : "text-veil-text-muted hover:text-veil-text-dim"
              )}
            >
              {f.label}
              {filter === f.key && (
                <span className="absolute bottom-0 left-2 right-2 h-px bg-veil-accent" />
              )}
            </button>
          ))}
        </div>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="mb-4 font-mono text-4xl text-veil-text-muted">
              ∅
            </div>
            <p className="font-serif text-lg text-veil-text-dim">
              No markets match this filter
            </p>
            <p className="mt-1 font-mono text-xs text-veil-text-muted">
              Check back later or browse all markets
            </p>
          </div>
        )}
      </div>
